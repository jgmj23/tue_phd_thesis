...
This manuscript is under review at the 26th International Conference on Algorithms and Architectures for Parallel Processing (ICA3PP 2026). The paper implements a real-time, post-quantum-secured image pipeline on an NVIDIA BlueField-3 DPU. It offloads both image I/O and the cryptographic stages (ML-KEM key establishment and ML-DSA authentication bootstrapping a symmetric per-frame data plane) from the host onto the DPU's Arm cores, ultimately delivering an inference-ready tensor to the GPU.
