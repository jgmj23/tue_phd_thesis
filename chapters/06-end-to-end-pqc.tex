...
To meet these budgets, the pipeline distributes work across heterogeneous hardware. General-purpose host CPUs traditionally handle network I/O and orchestration. Dedicated application accelerators, primarily GPUs, execute the dense matrix multiplications required for model training and inference. More recently, data processing units (DPUs) have been introduced to offload network packet processing and infrastructure tasks. The performance of the pipeline depends entirely on how effectively work is routed among these components to avoid stalls and contention.
...
The transition to post-quantum cryptography is often framed as a purely mathematical challenge: replacing vulnerable algorithms with quantum-resistant alternatives. However, as Part~\ref{part:pqc} has demonstrated, the true challenge of the post-quantum era is architectural. The inflated key sizes and heavy computational requirements of these new primitives threaten to overwhelm the host processors and application accelerators that power the modern datacenter.

Addressing this challenge requires moving cryptography out of the host and into the network infrastructure. By leveraging programmable data processing units (DPUs), the computational burden of post-quantum security can be isolated and accelerated. Whether offloading memory-bound code-based schemes to general-purpose Arm cores for predictable latency under contention, exploiting embedded GPUs to batch compute-heavy lattice schemes, or terminating an entire secure AI pipeline at network ingress to preserve application capacity, the central claim holds: post-quantum protection can be made practical at scale by treating security and the network device as a single co-designed data path. The DPU is no longer just a network interface; it becomes the boundary where the secure channel ends and the application begins.

Both chapters of Part~\ref{part:pqc} have so far treated the DPU as a cooperative and trustworthy substrate. Part~\ref{part:infra} steps back to interrogate that assumption. It examines the programmable datacenter fabric on which these mechanisms ultimately run, and the cross-layer risks that emerge when the network beneath a secure workload does not behave as its designers assumed.
