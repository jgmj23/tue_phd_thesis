...
Security proofs reason about an idealized machine: one that computes in constant time, emits nothing an observer can measure, and moves bytes over a channel that reveals only their ciphertext. Real deployments run on a physical substrate that violates every one of those assumptions. Transistors consume power as they switch, processors take data-dependent time, and packets traverse a shared fabric whose routing and timing are visible to anyone positioned to watch. This chapter treats that substrate not as a neutral medium but as an attack surface, from the device-level behaviour of individual chips to the datacenter-scale behaviour of the network fabric that connects them.
...
\section{Discussion}
\label{sec:rdma-discussion}

This chapter shows that cryptography cannot eliminate physical, metadata, or timing leaks. Cryptographic arguments successfully move the contest from payload content to observable structure, but they do not end it. The physical reality of the hardware and the optical fabric keeps structural information visible, and, worse, manipulable by an adversary positioned in the infrastructure.

Yet the same dynamic can be inverted. The structure that serves as an attack surface here becomes the defender's richest source of signal once payloads are opaque. Where this chapter reads fabric structure to attack a system, the next reads it to defend one: it turns the infrastructure from an attack surface into a sensor, using the very metadata that adversaries exploit to detect threats inside confidential AI clusters.
