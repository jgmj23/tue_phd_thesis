\chapter{A Cross-Layer Framework for Quantum-Resilient Communication}
\label{ch:background}
...
post-processing, post-quantum cryptographic acceleration, datacenter
infrastructure attacks, and confidential-AI observability. Read individually, they appear to belong to separate
research communities, each with its own models, metrics, and implicit assumptions.
What makes them a single dissertation is a shared \emph{method}: a cross-layer
view of secure communication in which a design choice at one layer is judged by its
consequences at the others.

Within that framework, the DPU recurs as an important
infrastructure mechanism rather than as a universal substrate. It executes QKD
post-processing and post-quantum workloads, terminates secure data paths, and
provides an observation point for confidential-AI systems. It is not, however, the
substrate of the photonic-device work or the optical-fabric attack analysis.

This chapter first defines the cross-layer lens and the analytical method that
recur throughout the thesis. It then introduces the DPU as a recurring
infrastructure substrate at the boundary between protocol execution and datacenter
deployment. The technical background specific to each topic, including integrated
...
where it can be developed alongside the corresponding contribution. The
...
makes the problem cross-layer is that physics-based and computational protections
must ultimately be integrated into the same communication system. Their guarantees
depend on interfaces between photonic devices, protocol processing, and programmable
network infrastructure, each of which imposes a different set of constraints.
...
\begin{figure}[htbp]
    ...
    \caption{The cross-layer security stack investigated in this dissertation. Reciprocal dependencies connect the device, protocol, and infrastructure layers. The DPU resides in the infrastructure layer, extends into the protocol layer through offloaded execution, and provides an infrastructure-side observation point.}
...
\section{The DPU as a Recurring Infrastructure Substrate}
\label{sec:bg-dpu}

The DPU is common to the QKD post-processing, PQC offload, secure AI-pipeline,
and confidential-AI observability contributions. It is introduced here once
because these chapters rely on the same underlying architecture, even though
the DPU is not part of every contribution in the dissertation.

Smart network interface cards (SmartNICs) and DPUs represent a paradigm shift in datacenter architecture. They extend
...
inline cryptographic engines for Advanced Encryption Standard--Galois/Counter Mode
(AES-GCM) and public-key acceleration), substantial
...
GPUDirect RDMA) that bypass the host CPU entirely
...
secured end-to-end AI pipelines manage image I/O (Chapter~\ref{ch:ai-pipelines}).
For this reason, the DPU is treated here as a shared implementation substrate
for these contributions rather than as a detail of any single chapter. Its
direct role is to connect protocol execution to infrastructure capabilities.
Device-layer behaviour reaches it indirectly through protocol data, control,
