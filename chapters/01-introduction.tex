...
This divergence in data lifespan mandates a tiered, heterogeneous approach to network security. A ``one-size-fits-all'' architecture is no longer viable when different secrets carry vastly different risk profiles. The massive volume of high- and medium-decay data requires computationally secure protection at datacenter scale, justifying the deployment of PQC accelerated by data processing units (DPUs). However, unconditionally high-value, low-decay secrets demand the potential for eternal security. Protecting these assets justifies the deployment complexity of physics-based security, such as QKD and integrated photonics. Consequently, mitigating the quantum threat requires a proactive, cross-layer re-engineering of communication systems well before the hardware to break them fully matures.
