...
\begin{figure}[htbp]
    ...
    modern cipher such as AES-256, by contrast, draws its key from
...
This divergence in data lifespan determines \emph{which} cryptographic mechanism a given secret warrants, and is the first reason a single, uniform answer to the quantum threat is insufficient. The massive volume of high- and medium-decay data requires computationally secure protection at datacenter scale, justifying the deployment of PQC accelerated by data processing units (DPUs). Unconditionally high-value, low-decay secrets, by contrast, demand the potential for eternal security, justifying the deployment complexity of physics-based security such as QKD and integrated photonics. Many deployments fall between these poles and combine mechanisms---hybrid classical/PQC or PQC/QKD key exchange---rather than committing to one outright.

Secret value, however, is not the only force pushing toward this heterogeneity. The migration is itself gradual and protracted: cryptographic standards are still maturing, confidence in newly standardized primitives remains incomplete, and large installed bases cannot be replaced overnight. The collapse of SIKE late in the NIST selection process is a reminder that a primitive long believed secure can fail abruptly.% TODO: citation (Castryck--Decru SIKE break, 2022)
These conditions reward crypto-agility, the prolonged coexistence of legacy and post-quantum mechanisms, and hybrid combinations adopted as a hedge against immature primitives rather than because of any particular secret's value. A ``one-size-fits-all'' architecture is therefore untenable for several compounding reasons at once. Consequently, mitigating the quantum threat requires a proactive, cross-layer re-engineering of communication systems well before the hardware to break them fully matures.

\section{Scope and Contributions}

\subsection{An Architectural Problem}

Selecting mechanisms by secret value and managing a protracted transition are
still not the whole problem. A cryptographic
...
therefore that scalable quantum-resilient communication is fundamentally an
integration challenge governed by the boundaries between the device, protocol, and
...
\begin{itemize}
\item \textbf{Chapter~\ref{ch:background}} establishes the two threads common to every contribution: the cross-layer method used to analyze them, and the DPU substrate on which they are built and observed. Topic-specific background is developed in each paper-centric chapter where it is needed.
