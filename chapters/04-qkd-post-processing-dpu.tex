...
In BB84, Alice seeks to share a secret key with Bob. She generates a sequence of random classical bits and encodes each bit into the polarization state of a single photon. Crucially, for each bit, she randomly chooses one of two non-orthogonal bases for the encoding: for instance, the rectilinear basis ($+$) or the diagonal basis ($\times$). A bit value of `0` might be encoded as horizontal ($|H\rangle$) or diagonal ($|D\rangle$) polarization, while a `1` is encoded as vertical ($|V\rangle$) or anti-diagonal ($|A\rangle$) polarization.

Alice transmits this stream of single photons to Bob over a quantum channel, such as a dedicated optical fiber. Upon receiving a photon, Bob must measure its state to recover the bit. However, because he does not know which basis Alice used, he randomly and independently chooses a measurement basis ($+$ or $\times$) for each incoming photon.
...
The quantum transmission concludes with Alice and Bob each holding a string of bits (Bob's measurements and Alice's original bits) and a corresponding array of the bases they used. At this point, the quantum physical layer's job is complete. However, this raw key is entirely unusable for cryptography: it is riddled with errors from basis mismatches, optical channel noise, and potential eavesdropping.

\section{The Computational Bottleneck of Post-Processing}
\label{sec:qkd-dpu-bottleneck}

Quantum Key Distribution (QKD) provides information-theoretic security by leveraging the fundamental principles of quantum mechanics to distribute symmetric cryptographic keys~\cite{Xu2020}. While the physical layer of QKD---the generation, transmission, and detection of quantum states discussed in Chapter~\ref{ch:quantum-sources}---has seen tremendous advancements, integrating these systems into modern datacenter architectures exposes a profound computational bottleneck.

The quantum channel only exchanges raw, error-prone measurement outcomes; it does not distribute a ready-to-use cryptographic key. A classical post-processing pipeline is required to distill these raw detections into secure, usable symmetric key material. This pipeline consists of several computation-heavy stages:
...
By executing QKD post-processing entirely within the DPU, the resulting quantum-secure keys never cross the PCIe bus to the host. Instead, they are formatted as Security Associations (SAs) and injected seamlessly into the DPU's local hardware IPsec or MACsec encryption engines. This enables quantum-secured RDMA (Q-RDMA). The data plane encryption operates transparently at line rate, while the continuous distillation of fresh keys occurs asynchronously on the DPU's Arm cores. Consequently, the fabric's latency and throughput remain completely isolated from the heavy computational cost of QKD key generation.

\section{Experimental Emulation of the Quantum Channel}
\label{sec:qkd-dpu-emulation}

Systematic evaluation of network cryptography requires stable and reproducible conditions. Physical QKD optical hardware is notoriously sensitive to environmental drift, dark counts, and afterpulsing, making it difficult to isolate post-processing performance from optical layer instability. 

To systematically stress-test the DPU architecture across varying error conditions, the implementation employs a synthetic BB84 source module on the DPU Arm cores. This module generates bit strings and basis arrays that precisely mimic the output of a physical quantum receiver, allowing the injection of reproducible QBER profiles. By decoupling the classical computing pipeline from the physical optical channel, the system can be rigorously benchmarked for sustained secret key rates, LDPC convergence, and IPsec key rotation overhead at varying error thresholds. While the qubit transmission layer is emulated, the entire post-processing pipeline, key derivation, and hardware IPsec injection are evaluated on real BlueField-3 silicon.
...
As the primary author, Joseph G. Meyer led the conceptualization of the DPU-accelerated architecture, implemented the custom Data Plane Development Kit (DPDK) application for the classical communication channel, and designed the hardware/software co-design strategy for LDPC decoding and IPsec key injection. The co-authors contributed to the theoretical modeling of the QBER profiles, the integration with the emulated DV-QKD physical layer, and the review of the experimental methodology.
