...
QKD leverages the fundamental principles of quantum mechanics---specifically, the no-cloning theorem and the observer effect---to establish a shared, secret cryptographic key between two parties, typically referred to as Alice and Bob~\cite{Xu2020}. Unlike classical public-key cryptography, which relies on the presumed computational hardness of mathematical problems, QKD provides information-theoretic security. Any attempt by an eavesdropper (Eve) to measure the quantum states in transit unavoidably disturbs them, introducing detectable errors. By monitoring the QBER, Alice and Bob can upper-bound the information leaked to Eve and distill a perfectly secure key through classical post-processing.
...
\begin{figure}[htbp]
  ...
  the quantum channel raises the QBER and is thereby
  detectable.}
...
\begin{enumerate}
    ...
    \item \textbf{Error Estimation:} A subset of the sifted bits is publicly compared to estimate the QBER. Because the protocol assumes all errors are caused by an eavesdropper, the QBER bounds the maximum information Eve could have gained.
    \item \textbf{Information Reconciliation:} To resolve discrepancies between Alice's and Bob's strings caused by channel noise, the parties perform forward error correction. This is typically achieved using rate-adaptive Low-Density Parity-Check (LDPC) codes~\cite{elkouss2011reconciliation} or the Cascade protocol~\cite{brassard1994cascade}. For LDPC, the receiver executes iterative belief propagation (e.g., the min-sum algorithm) to converge on the correct string.
    \item \textbf{Privacy Amplification:} Once the strings match, Alice and Bob compress them using a universal hash function (such as Toeplitz matrix multiplication) to erase any partial information the eavesdropper might hold, yielding the final, perfectly secret key~\cite{bennett1995privacy}.
\end{enumerate}
...
Datacenter networks operate at terabit-per-second scales, relying heavily on low-latency, high-throughput transport protocols. Traditionally, QKD post-processing has been executed either on rigid field-programmable gate array (FPGA) appliances or on general-purpose host CPUs. Both integration models suffer from structural flaws when deployed in high-performance environments.
...
This manuscript was accepted and will be presented at the 2026 IEEE International Conference on Quantum Computing and Engineering (QCE), also known as IEEE Quantum Week. The paper contributes a complete hardware/software co-design strategy for offloading the classical communication channel, LDPC decoding, and IPsec key injection onto a DPU. This demonstrates that continuous distillation of fresh keys can occur asynchronously on programmable network hardware without burdening the host CPU.

Beyond this article, the work on quantum-secured networking also contributed to several filed patent applications, \enquote{Multi-Hybrid Cryptographic Key Exchange (MHKE)} (I1) and \enquote{ROSE-QKD: Receiver-Only, Semi-Honest Emitter Quantum Key Distribution} (I2), together with a draft application on \enquote{Capability-Based Authentication of Quantum Nodes via Statistical Analysis} (I3). These inventions, which extend the architectural concepts evaluated in this chapter, are reported at title level among the patents in the List of Publications.
...
\section{Part I Epilogue}

Part~\ref{part:qkd} has traced quantum-secured networking end to end: from the integrated photonic sources of Chapter~\ref{ch:quantum-sources} to the post-processing pipeline of this chapter. Yet the very information-theoretic guarantee that motivates QKD comes bundled with practical limitations that bound where it can realistically be deployed:
\begin{itemize}
    \item \textbf{It is only a key-exchange primitive, and cannot bootstrap its own trust.} QKD distributes shared secret bits and nothing more, so the key must still feed a classical symmetric cipher, and the public channel must be authenticated by a pre-shared secret or a computationally secure signature. Its ``unconditional'' security thus rests, at bootstrap, on a classical assumption. This matters less than it appears: authentication cannot be broken \emph{retroactively}---unlike confidentiality, where an adversary can harvest ciphertext today and decrypt it once a quantum computer arrives, a man-in-the-middle attack must succeed \emph{in real time}. A merely computationally secure authenticator need only hold for the lifetime of the exchange, and after the first round QKD can authenticate itself from its own output. The classical dependency is an ephemeral bootstrap, not a standing vulnerability.
    \item \textbf{It is physically constrained.} QKD demands dedicated optical hardware and low-loss fiber (or free-space line of sight) per link; it is distance-limited, since the no-cloning theorem forbids amplification and photon loss caps the key rate; and it is inherently point-to-point. Extending reach or fanning out to many users today relies on trusted relay nodes, which reintroduce the very points of trust QKD sought to remove.
    \item \textbf{Its theoretical security exceeds its implementation security.} Real devices deviate from their idealized models, opening side-channel and hardware attacks (detector blinding, Trojan-horse probing, imperfect state preparation). More broadly, QKD is a radically new security paradigm, and the cryptographic community---rightly conservative---adopts such foundations only slowly, after years of scrutiny and standardization.
\end{itemize}

These constraints make it impractical to secure every network segment with QKD alone. Comprehensive quantum-safe security therefore demands a complementary, software-deployable approach: PQC---classical algorithms believed to resist quantum attack, requiring no special optics and slotting into existing protocols. Part~\ref{part:pqc} takes up this approach, shifting from quantum-generated key material to the execution of PQC algorithms on the same programmable network infrastructure used here for QKD post-processing.
